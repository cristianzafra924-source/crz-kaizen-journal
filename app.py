  ...
  <span style="font-size:10px;color:#475569;margin-left:auto;">{_utc_str}</span>
    ...
    with _col_right:
        ...
        _panel_html = (
            ...
            '.chtabs{display:flex;gap:3px;padding:7px;background:#060d1a;'
            ...
            '.note{font-size:9px;color:#475569;padding:2px 9px;flex-shrink:0;'
            'border-bottom:1px solid #0f1f35;min-height:16px;background:#060d1a}'
            '.sectabs{display:flex;gap:0;flex-shrink:0;border-bottom:1px solid #0f1f35}'
            '.stab{flex:1;padding:5px 0;text-align:center;font-size:9px;font-weight:700;'
            'letter-spacing:.08em;cursor:pointer;color:#475569;background:#060d1a;'
            'border-right:1px solid #0f1f35;transition:all .12s}'
            '.stab:last-child{border-right:none}'
            '.stab:hover{color:#e2e8f0}'
            '.stab.on{color:#2dd4bf;background:#0a1020;border-bottom:2px solid #2dd4bf}'
            # Filter bar
            '.filters{display:flex;flex-wrap:wrap;gap:3px;padding:5px 7px;'
            'flex-shrink:0;border-bottom:1px solid #0f1f35;background:#060d1a}'
            '.filt{padding:2px 8px;font-size:8px;font-weight:700;border-radius:99px;'
            'cursor:pointer;border:1px solid #1e2a3a;color:#475569;background:#0a1020;'
            'transition:all .1s;white-space:nowrap}'
            '.filt:hover{color:#e2e8f0;border-color:#475569}'
            '.filt.on{color:#060d1a;background:#2dd4bf;border-color:#2dd4bf}'
            # Feed
            '.feed{flex:1;overflow-y:auto;padding:4px 8px 8px;display:flex;flex-direction:column}'
            ...
            'border-radius:5px;border-left:3px solid #475569;flex-shrink:0}'
            ...
            '.tw-sec{flex:1;overflow:hidden;display:none}'
            '.tw-sec iframe{width:100%;height:100%;border:none}'
            '</style>'
            '</head><body>'
            ...
            '<div class="sectabs">'
            '  <div class="stab on" id="stab-news" onclick="showSec(\'news\')">NOTICIAS GLOBALES</div>'
            '  <div class="stab" id="stab-tw" onclick="showSec(\'tw\')">𝕏 TRUMP LIVE</div>'
            '</div>'
            # Filter buttons (only visible in news tab)
            '<div class="filters" id="filters">'
            '  <div class="filt on" data-k="" onclick="setFilt(this,\'\')">Todo</div>'
            '  <div class="filt" data-k="guerra|war|conflict|ataque|attack|strike" onclick="setFilt(this,this.dataset.k)">Guerra</div>'
            '  <div class="filt" data-k="ucrania|ukraine|rusia|russia|putin" onclick="setFilt(this,this.dataset.k)">Ucrania</div>'
            '  <div class="filt" data-k="israel|gaza|palestin|libano|siria|hamas|iran" onclick="setFilt(this,this.dataset.k)">Oriente Medio</div>'
            '  <div class="filt" data-k="petroleo|petróleo|oil|gas|energ" onclick="setFilt(this,this.dataset.k)">Energía</div>'
            '  <div class="filt" data-k="sanción|sancion|sanction|arancel|tariff" onclick="setFilt(this,this.dataset.k)">Sanciones</div>'
            '  <div class="filt" data-k="trump|biden|eeuu|estados unidos|white house" onclick="setFilt(this,this.dataset.k)">EEUU</div>'
            '</div>'
            '<div class="feed" id="sec-news"><div style="color:#334155;font-size:10px;padding:12px;">Cargando...</div></div>'
            '<div class="tw-sec" id="sec-tw">'
            '  <iframe id="tw-frame" src=""></iframe>'
            '</div>'
            '<script>'
            'var CHS=' + _ch_json + ';'
            'var ALL_ARTS=[];'
            ...
            '  t.onclick=function(){'
            '    document.querySelectorAll(".chtab").forEach(function(x){x.classList.remove("active");});'
            ...
            'function showSec(s){'
            '  var isNews=s==="news";'
            '  document.getElementById("sec-news").style.display=isNews?"flex":"none";'
            '  document.getElementById("sec-tw").style.display=isNews?"none":"flex";'
            '  document.getElementById("filters").style.display=isNews?"flex":"none";'
            '  document.getElementById("stab-news").className="stab"+(isNews?" on":"");'
            '  document.getElementById("stab-tw").className="stab"+(!isNews?" on":"");'
            '  if(!isNews&&!_twLoaded){'
            '    document.getElementById("tw-frame").src='
            '      "https://syndication.twitter.com/srv/timeline-profile/screen-name/realDonaldTrump?dnt=true&theme=dark&chrome=noheader%20nofooter%20noborders%20transparent";'
            '    _twLoaded=true;'
            '  }'
            '}'
            # Filter logic
            'function setFilt(el,k){'
            '  document.querySelectorAll(".filt").forEach(function(x){x.classList.remove("on");});'
            '  el.classList.add("on");_curFilt=k;renderFeed();'
            '}'
            'function matchFilt(title,k){'
            '  if(!k)return true;'
            '  var t=(title||"").toLowerCase();'
            '  var parts=k.split("|");'
            '  for(var i=0;i<parts.length;i++)if(t.indexOf(parts[i])>=0)return true;'
            '  return false;'
            '}'
            'function getC(t){'
            '  t=(t||"").toLowerCase();'
            '  var KW={guerra:"#ef4444",war:"#ef4444",conflicto:"#ef4444",conflict:"#ef4444",'
            '    ataque:"#ef4444",attack:"#ef4444",strike:"#ef4444",muerto:"#ef4444",kill:"#ef4444",'
            '    militar:"#f97316",military:"#f97316",misil:"#f97316",missile:"#f97316",'
            '    "sanción":"#f97316",sanction:"#f97316",arancel:"#f97316",'
            '    "petróleo":"#eab308",oil:"#eab308",gas:"#eab308","energía":"#eab308",energy:"#eab308",'
            ...
            '  for(var k in KW)if(t.indexOf(k)>=0)return KW[k];return "#475569";'
            '}'
            ...
            'function renderFeed(){'
            '  var feedEl=document.getElementById("sec-news");'
            '  var arts=ALL_ARTS.filter(function(a){return matchFilt(a.title,_curFilt);});'
            '  if(!arts.length){feedEl.innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Sin resultados</div>\';return;}'
            '  var h="";'
            '  arts.forEach(function(a){'
            '    var c=getC(a.title),tm=ago(a.pub);'
            ...
            '  feedEl.innerHTML=h;'
            '}'
            # Fetch news in Spanish via allorigins
            'var feedEl=document.getElementById("sec-news");'
            'var newsUrl=encodeURIComponent("https://news.google.com/rss/search?q=guerra+conflicto+ucrania+israel+iran+sanciones+militar+ataque&hl=es&gl=ES&ceid=ES:es");'
            'fetch("https://api.allorigins.win/get?url="+newsUrl)'
            '.then(function(r){return r.json();})'
            '.then(function(d){'
            '  if(!d.contents){feedEl.innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Sin noticias</div>\';return;}'
            '  var parser=new DOMParser();'
            '  var xml=parser.parseFromString(d.contents,"text/xml");'
            '  var items=xml.querySelectorAll("item");'
            '  if(!items.length){feedEl.innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Sin noticias</div>\';return;}'
            '  ALL_ARTS=[];'
            ...
            '    ALL_ARTS.push({title:title,url:link,src:src,pub:pub});'
            '  });'
            '  renderFeed();'
            '})'
            '.catch(function(){feedEl.innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Error al cargar noticias</div>\';});'
            '</script></body></html>'
        )
