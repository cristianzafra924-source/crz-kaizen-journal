  ...
  <span style="font-size:10px;color:#475569;margin-left:auto;">{_utc_str}</span>
    ...
    with _col_right:
        ...
        _panel_html = (
            ...
            '.tw-sec{flex:1;overflow-y:auto;display:none;background:#060d1a;padding:0}'
            ...
            '<span class="filt" data-k="guerra,war,conflict,ataque,attack,strike,muerto,kill" onclick="filt(this)">Guerra</span>'
            ...
            '<span class="filt" data-k="petroleo,oil,gas,energia,energy" onclick="filt(this)">Energia</span>'
            '<span class="filt" data-k="sancion,sanction,arancel,tariff" onclick="filt(this)">Sanciones</span>'
            '<span class="filt" data-k="trump,eeuu,estados unidos,washington,biden" onclick="filt(this)">EEUU</span>'
            '</div>'
            ...
            '  t.onclick=function(){document.querySelectorAll(".chtab").forEach(function(x){x.classList.remove("active");});'
            ...
            'function getC(t){'
            '  t=(t||"").toLowerCase();'
            '  var m={guerra:"#ef4444",war:"#ef4444",conflict:"#ef4444",attack:"#ef4444",strike:"#ef4444",'
            '    kill:"#ef4444",muerto:"#ef4444",militar:"#f97316",military:"#f97316",missile:"#f97316",'
            ...
            '  });el.innerHTML=h;'
            '}'
            'var newsUrl=encodeURIComponent("https://news.google.com/rss/search?q=war+conflict+ukraine+israel+iran+military+sanctions+attack&hl=en&gl=US&ceid=US:en");'
            ...
            '  if(!items.length){document.getElementById("sec-news").innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Sin noticias</div>\';return;}'
            '  items.forEach(function(it){'
            '    ALL.push({'
            '      title:(it.querySelector("title")||{textContent:""}).textContent,'
            '      url:(it.querySelector("link")||{textContent:"#"}).textContent,'
            '      src:(it.querySelector("source")||{textContent:""}).textContent,'
            '      pub:(it.querySelector("pubDate")||{textContent:""}).textContent'
            '    });'
            '  });render();'
            '})'
            '.catch(function(){document.getElementById("sec-news").innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Error al cargar</div>\';});'
            '</script></body></html>'
        )
