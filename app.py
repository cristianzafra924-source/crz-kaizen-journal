  ...
  <span style="font-size:10px;color:#475569;margin-left:auto;">{_utc_str}</span>
    ...
    with _col_right:
        ...
        _panel_html = (
            ...
            'cursor:pointer;color:#475569;background:#060d1a;border-right:1px solid #0f1f35;transition:color .12s}'
            ...
            '.tw-sec{flex:1;overflow-y:auto;display:none;background:#060d1a}'
            '.tw-sec iframe{width:100%;height:100%;border:none;display:block}'
            '</style></head><body>'
            ...
            '<span class="filt" data-k="guerra,war,conflict,ataque,attack,strike,muerto,killed" onclick="filt(this)">Guerra</span>'
            '<span class="filt" data-k="ucrania,ukraine,rusia,russia,putin,zelenski" onclick="filt(this)">Ucrania</span>'
            '<span class="filt" data-k="israel,gaza,palestin,libano,hamas,iran,siria,hezbollah" onclick="filt(this)">Or.Medio</span>'
            '<span class="filt" data-k="petroleo,petróleo,oil,gas,energía,energia,energy" onclick="filt(this)">Energía</span>'
            '<span class="filt" data-k="sanción,sancion,sanction,arancel,tariff" onclick="filt(this)">Sanciones</span>'
            '<span class="filt" data-k="trump,eeuu,estados unidos,washington,fed,reserva federal" onclick="filt(this)">EEUU</span>'
            '</div>'
            '<div class="feed" id="sec-news"><div style="color:#334155;font-size:10px;padding:12px;">Cargando noticias en español...</div></div>'
            ...
            '  yt.src=ch.videoId&&ch.videoId.length===11'
            ...
            '  note.textContent=ch.videoId&&ch.videoId.length===11?"EN VIVO":"Stream directo";'
            '}'
            'if(CHS.length>0)loadVideo(CHS[0]);'
            # Section tabs
            'function showSec(s){'
            '  var n=s==="news";'
            '  document.getElementById("sec-news").style.display=n?"block":"none";'
            '  document.getElementById("sec-tw").style.display=n?"none":"block";'
            '  document.getElementById("filtersbar").style.display=n?"flex":"none";'
            '  document.getElementById("stab-news").className="stab"+(n?" on":"");'
            '  document.getElementById("stab-tw").className="stab"+(!n?" on":"");'
            '}'
            # Filter
            'function filt(el){'
            ...
            '    "ataque":"#ef4444","attack":"#ef4444","strike":"#ef4444","muerto":"#ef4444","killed":"#ef4444",'
            '    "militar":"#f97316","military":"#f97316","misil":"#f97316","missile":"#f97316",'
            '    "sanción":"#f97316","sancion":"#f97316","sanction":"#f97316","arancel":"#f97316","tariff":"#f97316",'
            '    "petróleo":"#eab308","petroleo":"#eab308","oil":"#eab308","gas":"#eab308","energía":"#eab308","energy":"#eab308",'
            '    "irán":"#ef4444","iran":"#ef4444","rusia":"#f97316","russia":"#f97316","ukraine":"#f97316","ucrania":"#f97316",'
            ...
            '}'
            # Time formatting - shows HH:MM and relative
            'function fmtTime(ds){'
            '  if(!ds)return "";'
            '  try{'
            '    var d=new Date(ds);'
            ...
            '  });el.innerHTML=h;'
            '}'
            # Fetch Spanish RSS via allorigins
            'var rsses=['
            '  encodeURIComponent("https://news.google.com/rss/search?q=guerra+conflicto+ucrania+israel+iran+sanciones+militar+ataque&hl=es&gl=ES&ceid=ES:es"),'
            '  encodeURIComponent("https://news.google.com/rss/search?q=guerra+conflicto+geopolitica+mercados+economia&hl=es&gl=ES&ceid=ES:es")'
            '];'
            'function fetchRss(idx){'
            '  if(idx>=rsses.length){if(!ALL.length)document.getElementById("sec-news").innerHTML=\'<div style="color:#334155;font-size:10px;padding:12px;">Sin noticias disponibles</div>\';return;}'
            '  fetch("https://api.allorigins.win/get?url="+rsses[idx])'
            ...
            '      if(title)ALL.push({title:title,url:link,src:src,pub:pub});'
            '    }'
            '    if(ALL.length)render();'
            '    else fetchRss(idx+1);'
            '  })'
            '  .catch(function(){fetchRss(idx+1);});'
            '}'
            'fetchRss(0);'
            '</script></body></html>'
        )
